...
const getNews = async function () {
  const API_KEY = "55e3f1fdc1c94581b6b11537c9b8f8b";
  const url =
  ...
  try {
    ...
    if (!res.ok) {
      isError = true;
      // throw new Error(`Something went wrong: ${res.status} `); //! burada hata fırlattık ve catch  egirdi sonra kodu block ladı bizde bunun olmaması ve aşağıdaki function nun çalışması için yorua aldık
    }
